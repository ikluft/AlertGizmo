...
This module goes through HTML files generated by previous passes and makes a PNG image of their rendered contents.
Usually in each run, there is expected to be only one HTML file generated, whichever module is run. This plugin can be included on the list of postprocessing classes to run if HTML to PNG rendering is desired for that content.
